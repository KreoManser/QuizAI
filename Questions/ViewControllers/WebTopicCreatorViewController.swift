...
class WebTopicCreatorViewController: UIViewController, WKNavigationDelegate {
	...
	@IBAction func outputBarButtonAction(_ sender: UIBarButtonItem) {
		...
		webView.multipleInputValuesFrom(ids: inputFieldIDs) { values in
			...
			self.webView.checkedCheckboxes(ids: checkboxFieldsIDs) { checkedCheckboxes in
				let questionsInAIOrder = checkedCheckboxes[0]
				...
				let options = Topic.Options(name: name, timePerSetInSeconds: timePerSetInSeconds, helpButtonEnabled: helpButtonEnabled, questionsInRandomOrder: questionsInAIOrder, showCorrectIncorrectAnswer: showCorrectIncorrectAnwer, multipleCorrectAnswersAsMandatory: multipleCorrectAnswersAsMandatory)
				...
				for i in 1...numberOfSets {
					for j in 1...questionsPerSet {
                        ...
                        let difficultyID = "question-difficulty-\(i)-\(j)"
                        ...
                        questionAndAnswersIDs.append((questionID: questionID, questionImageID: questionImageID, difficultyID: difficultyID, answerIDs: answerIDs))
					}
				}
	...
	private func setupWebView() {
		...
		let bgColor: UIColor = .themeStyle(
            dark: .black,
            light: UIColor(RGBred: 239, green: 239, blue: 244)
        )
	...
	private func topicActionAlert(topic: Topic) {
		...
		whatToDoAlertController.addAction(title: L10n.TopicsCreation_Alerts_Save_Accept, style: .default) { _ in
            ToastAlert.present(
                onSuccess: L10n.TopicsCreation_Alerts_Save_Success,
                onError: L10n.TopicsCreation_Alerts_Save_Error,
                withLength: .short,
                in: self,
                operation: {
                    return SetOfTopics.shared
                        .save(topic: TopicEntry(name: topic.options?.name ?? "", content: topic))
                }
            )
		}
		...
		whatToDoAlertController.addAction(title: L10n.TopicsCreation_Alerts_Save_Share, style: .default) { _ in
            ...
            _ = min(self.view.bounds.width, self.view.bounds.height)
