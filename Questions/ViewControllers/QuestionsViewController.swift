...
class QuestionsViewController: UIViewController {
    ...
    var quizModel: QuizPredictor!
    var askedQuestions: Set<Int> = []
    ...
    private func loadModel() {
        ...
        quizModel = try? QuizPredictor(configuration: configuration)
        if quizModel == nil {
            print("Ошибка загрузки модели QuizPredictor.")
        }
    }
	...
	private func preloadImages() {
		for fullQuestion in self.set.dropFirst() {
    ...
    func getNextQuestion(currentCategory: Int, correctAnswerIndex: UInt8, userAnswerIndex: UInt8) {
        let modelInput = QuizPredictorInput(currentCategory: Int64(currentCategory), correctAnswer: Int64(correctAnswerIndex), userAnswer: Int64(userAnswerIndex))

        guard let prediction = try? quizModel.prediction(input: modelInput) else {
            print("Не удалось получить предсказание от модели")
            return
        }

        let predictedCategory = Int(prediction.nextCategory)

        if let nextQuestion = set.first(where: { question in
            let questionCategory = Int(question.difficulty ?? 0)
            return questionCategory == predictedCategory && !askedQuestions.contains(question.id.getId())
        }) {
            askedQuestions.insert(nextQuestion.id.getId())
    ...
    func displayQuestion(_ question: Question) {
        DispatchQueue.main.async {
            ...
            self.answerButtons.enumerated().forEach { index, button in
	...
	@objc private func verify(answer: UInt8) {
        ...
        let currentCategory = set.first?.difficulty ?? 1
        let correctAnswer = Int(correctAnswersSet.first ?? 0)
        ...
        getNextQuestion(currentCategory: currentCategory, correctAnswerIndex: UInt8(correctAnswer), userAnswerIndex: answer)
